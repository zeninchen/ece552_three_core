...
module hart #(
    ...
    wire memory_wait;
    wire fetch_wait;
    wire flush_fetch; // for the flush signal in fetch stage, we can just use the i_not_pc_4 signal from execute stage, because when a flush happens, the i_not_pc_4 signal will be high, and we can use it to determine whether to flush the instruction in fetch stage or not
    //forwarding signals
    ...
    reg [31:0] if_id_pc_4_before_predict;
    reg [31:0] id_ex_pc_4_before_predict;
    wire [31:0] if_pc_4_before_predict;
    ...
    // -- icache CPU-side wires --
    wire        icache_busy;
    wire [31:0] icache_res_rdata;
    wire        icache_req_ren;   // = what fetch wants to send as imem_ren
    // icache external memory side
    wire        icache_mem_ready;
    wire [31:0] icache_mem_addr;
    wire        icache_mem_ren;
    wire        icache_mem_wen;   // icache never writes (read-only)
    wire [31:0] icache_mem_wdata; // unused for icache
    wire [31:0] icache_mem_rdata;
    wire        icache_mem_valid;

    // -- dcache CPU-side wires --
    wire        dcache_busy;
    wire [31:0] dcache_res_rdata;
    wire        dcache_req_ren;
    wire        dcache_req_wen;
    reg  dcache_req_was_sent;
    wire dcache_resp_valid;

    reg  dcache_busy_q;
    wire dcache_hit_done;
    wire dcache_miss_done;
    // add for sw followed by lw instruction
    reg [31:0] dcache_req_addr_q;
    reg [3:0]  dcache_req_mask_q;
    reg [31:0] dcache_req_wdata_q;
    // dcache external memory side
    ...
    // ---- icache: CPU side driven by fetch ----
    assign icache_mem_ready = i_imem_ready;
    assign icache_mem_rdata = i_imem_rdata;
    assign icache_mem_valid = i_imem_valid;
    assign o_imem_raddr     = icache_mem_addr; // address actually sent to memory comes from cache
    ...
    cache u_icache (
        ...
        .o_mem_wen    (icache_mem_wen),   // unused, icache is read-only
        .o_mem_wdata  (icache_mem_wdata), // unused
        ...
        .i_req_wen    (1'b0),             // instruction cache never writes
        .i_req_mask   (4'b1111),
        .i_req_wdata  (32'd0),
    ...
    assign icache_valid = !icache_busy;
    // phase 1: fetch
    wire hold_fetch = stall_decode || stall_fetch || stall_mem; // when there's a hazard, we need to hold the fetch stage to stall the decode stage and insert nop in execute stage, so we can just use the stall signals from hazard detection to determine whether to hold the fetch stage or not
	wire fetch_accept;
    fetch #(.RESET_ADDR(RESET_ADDR)) u_fetch (
        ...
        .i_id_ex_pc_4_before_predict(id_ex_pc_4_before_predict), // for branch predictor update in fetch stage, to determine the correct next PC for the instruction in execute stage (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine the correct next PC for the instruction in execute stage)
        .i_id_ex_predict_taken(id_ex_predicted_taken), // for branch predictor update in fetch stage, to determine whether the instruction in execute stage is a taken branch or not (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine whether the instruction in execute stage is a taken branch or not)
        .i_is_pc_4     (ex_is_pc_4), // for next PC calculation in hart (for non-control-transfer instructions)
        .i_not_pc_4    (ex_not_pc_4), // for next PC calculation in hart (for non-control-transfer instructions)
        .i_stall        (hold_fetch), // for hazard detection to stall the fetch stage when there's a hazard
        ...
        // Connect to iCache
        .i_imem_ready (~icache_busy),
        .o_imem_ren   (icache_req_ren),
        .i_imem_valid (icache_valid),   //need to change?
        .o_fetch_wait    (fetch_wait),
        .o_fetch_accept   (fetch_accept),
        .o_flush_fetch    (flush_fetch), // for the flush signal in fetch stage, we can just use the i_not_pc_4 signal from execute stage, because when a flush happens, the i_not_pc_4 signal will be high, and we can use it to determine whether to flush the instruction in fetch stage or not
        .id_ex_Branch(id_ex_Branch),
        .id_ex_valid(id_ex_valid), // for the flush signal in fetch stage, we can just use the i_not_pc_4 signal from execute stage, because when a flush happens, the i_not_pc_4 signal will be high, and we can use it to determine whether to flush the instruction in fetch stage or not
        .id_ex_Jalr(id_ex_Jalr), // for the flush signal in fetch stage, we can just use the i_not_pc_4 signal from execute stage, because when a flush happens, the i_not_pc_4 signal will be high, and we can use it to determine whether to flush the instruction in fetch stage or not
        .o_pc_4_before_predict(if_pc_4_before_predict), // for branch predictor update in fetch stage, to determine the correct next PC for the instruction in execute stage (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine the correct next PC for the instruction in execute stage
        .o_predict_taken(if_predict_taken) // for branch predictor update in fetch stage, to determine whether the instruction in execute stage is a taken branch or not (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine whether the instruction in execute stage is a taken branch or not)
    );

    assign if_instr = icache_res_rdata; //from iCache
    ...
    reg if_id_valid;
    // use icache for decode_load
    wire decode_load = !stall_decode && icache_valid && !flush_fetch; // when not stalled and imem is valid and not flushed, we can update the IF/ID pipeline register, otherwise we hold the IF/ID pipeline register until the stall is over or the imem is valid or the flush is over
    always @(posedge i_clk) begin
        if (i_rst||nop_id || flush_fetch) begin
            if_id_pc   <= RESET_ADDR;
            if_id_pc_4 <= RESET_ADDR + 32'd4;
            if_id_inst <= 32'h00000013; // NOP = addi x0, x0, 0
            if_id_valid <= 1'b0; // if it's a nop, then it's not valid, if it's not a nop, then it's valid
            if_id_predicted_taken <= 1'b0; // when reset or nop, we set the predicted taken to 0, because we don't want to predict taken for a nop instruction, and we also don't want to predict taken when reset, because we want to start from a clean state for the branch predictor
            if_id_pc_4_before_predict <= 32'd0; // when reset or nop
            //if hazard detected stall the decode stage to hold the instruction, and insert the nop instruction into the execute stage
        end else if (stall_decode || stall_fetch || stall_mem) begin
            if_id_pc    <= if_id_pc;
            if_id_pc_4  <= if_id_pc_4;
            if_id_inst  <= if_id_inst;
            if_id_valid <= if_id_valid;
            if_id_predicted_taken <= if_id_predicted_taken;
            if_id_pc_4_before_predict <= if_id_pc_4_before_predict;
        end else if (fetch_accept) begin
            if_id_predicted_taken <= if_predict_taken; // for branch predictor update in fetch stage, to determine whether the instruction in execute stage is a taken branch or not (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine whether the instruction in execute stage is a taken branch or not)
            if_id_pc_4_before_predict <= if_pc_4_before_predict; // for branch predictor update in fetch stage, to determine the correct next PC for the instruction in execute stage (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine the correct next PC for the instruction in execute stage)
            if_id_pc   <= if_pc;
            if_id_pc_4 <= if_pc_4;
            if_id_inst <= if_instr;
            if_id_valid <= 1'b1;
    ...
    always @(posedge i_clk) begin
        ...
        if (i_rst||nop_ex) begin
            id_ex_valid <= 1'b0; // if it's a nop, then it's not valid, if it's not a nop, then it's valid
            id_ex_pc         <= 32'd0;
            id_ex_pc_4       <= 32'd0;
            ...
            id_ex_RegWrite   <= 1'b0;
            id_ex_Mask       <= 4'd0;
            id_ex_predicted_taken <= 1'b0; // for branch predictor update in execute stage, to determine whether the instruction in execute stage is a taken branch or not (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine whether the instruction in execute stage is a taken branch or not)
            id_ex_pc_4_before_predict <= 32'd0; // for branch predictor update in execute stage, to determine the correct next PC for the instruction in execute stage (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine the correct next PC for the instruction in execute stage)
            //if not stalled, 
            ...
            id_ex_MemRead    <= id_MemRead && if_id_valid;
            id_ex_MemWrite   <= id_MemWrite && if_id_valid; // only write when the instruction is valid
            id_ex_WriteRegSrc<= id_WriteRegSrc;

            id_ex_ALUOp      <= id_ALUOp;
            id_ex_ALUSrcA    <= id_ALUSrcA;
            id_ex_ALUSrcB    <= id_ALUSrcB;
            id_ex_RegWrite   <= id_RegWrite && if_id_valid; // only write when the instruction is valid
            id_ex_Mask       <= id_Mask;
            id_ex_predicted_taken <= if_id_predicted_taken; // for branch predictor update in execute stage, to determine whether the instruction in execute stage is a taken branch or not (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine whether the instruction in execute stage is a taken branch or not)
            id_ex_pc_4_before_predict <= if_id_pc_4_before_predict; // for
        end 
    end
    ...
    // ---- dcache: CPU side driven by memory_stage ----
    assign dcache_mem_ready = i_dmem_ready;
    assign dcache_mem_rdata = i_dmem_rdata;
    assign dcache_mem_valid = i_dmem_valid;
    // The actual o_dmem_* ports now come from the dcache's memory side.
    // We override the direct assignments to o_dmem_addr/ren/wen below.
    assign o_dmem_addr  = dcache_mem_addr;
    assign o_dmem_ren   = dcache_mem_ren;
    assign o_dmem_wen   = dcache_mem_wen;
    assign o_dmem_wdata = dcache_mem_wdata; // dcache provides write data to mem
    ...
    cache u_dcache (
        ...
        .o_busy       (dcache_busy),
        // .i_req_addr   (dcache_req_was_sent ? dcache_req_addr_q  : ex_mem_alu_result),
        .i_req_addr   (ex_mem_alu_result),
        .i_req_ren    (dcache_req_ren),
        .i_req_wen    (dcache_req_wen),
        // .i_req_mask   (dcache_req_was_sent ? dcache_req_mask_q  : ex_mem_Mask),
        // .i_req_wdata  (dcache_req_was_sent ? dcache_req_wdata_q : ex_mem_store_data),
        .i_req_mask   (ex_mem_Mask),
        .i_req_wdata  (ex_mem_store_data),
    ...
    always @(posedge i_clk) begin
        if (i_rst)
            ...
            dcache_busy_q <= dcache_busy;
    end

    assign dcache_hit_done  = (dcache_req_ren || dcache_req_wen) && !dcache_busy;
    assign dcache_miss_done = dcache_busy_q && !dcache_busy;
    assign dcache_resp_valid = dcache_hit_done || dcache_miss_done;

    always @(posedge i_clk) begin
        if (i_rst||nop_mem) begin
            ex_mem_valid <= 1'b0; // if it's a nop, then it's not valid, if it's not a nop, then it's valid
            ...
            ex_mem_MemWrite    <= ex_MemWrite && id_ex_valid; // only write when the instruction is valid
            ex_mem_Mask        <= ex_Mask;
            ex_mem_RegWrite    <= id_ex_RegWrite && id_ex_valid; // only write when the instruction is valid
        ...
        // else ex_mem_valid <= 1'b0; // if stalled, we need to invalidate the instruction in the memory stage, because it's not finished yet, and we will insert the nop instruction into the memory stage in the next cycle, so we need to invalidate the instruction in the memory stage to avoid any side effect from the instruction in the memory stage.
    end
    ...
    assign o_dmem_mask = 4'b1111; //cache to memory mask is always 4'b1111
    ...
    memory_stage u_memory (
        ...
        .i_dmem_ready(~dcache_busy),
        .i_dmem_valid(dcache_resp_valid),
        .o_dmem_ren(dcache_req_ren),
        .o_dmem_wen(dcache_req_wen),
    ...
    always @(posedge i_clk) begin
        if (i_rst) begin
            mem_wb_valid <= 1'b0; // if it's a nop, then it's not valid, if it's not a nop, then it's valid
            ...
            mem_wb_RegWrite <= ex_mem_RegWrite && ex_mem_valid; //only write when the instruction is valid.
            if (ex_mem_MemRead && dcache_resp_valid && ex_mem_valid)
                mem_wb_mem_read_data <= dcache_res_rdata;
        // if (!i_rst && !nop_wb && dcache_resp_valid && !stall_mem && ex_mem_MemRead) begin    //use dcache
        //     mem_wb_mem_read_data <= dcache_res_rdata;
        end
    end
	...
	assign o_retire_valid     = ~(wb_inst == 32'h00000013) && mem_wb_valid && !stall_mem; //when the instruction is not nop
    ...
    // Only expose real source registers to hazard detection.
    // This avoids false load-use stalls from immediate/unused instruction fields.
    wire [6:0] if_id_opcode = if_id_inst[6:0];

    wire if_id_uses_rs1 = if_id_valid && (
    ...
    wire if_id_uses_rs2 = if_id_valid && (
        (if_id_opcode == 7'b0110011) || // R-type
        (if_id_opcode == 7'b0100011) || // store
        (if_id_opcode == 7'b1100011)    // branch
    );

    wire [4:0] hazard_if_id_rs1 =
        if_id_uses_rs1 ? if_id_inst[19:15] : 5'd0;

    wire [4:0] hazard_if_id_rs2 =
        if_id_uses_rs2 ? if_id_inst[24:20] : 5'd0;
    //instantiate hazard_detect.v
    hazard_detect u_hazard_detect (
        // .i_if_idrs1_addr (if_id_inst[19:15]),
        // .i_if_idrs2_addr (if_id_inst[24:20]),
        .i_if_idrs1_addr (hazard_if_id_rs1),
        .i_if_idrs2_addr (hazard_if_id_rs2),
        ...
        .i_ex_mem_MemWrite(ex_mem_MemWrite),
        .i_imem_valid(icache_valid),    //use cache
        .i_dmem_valid(dcache_resp_valid),
        .i_dmem_ready(1'b0),
        .i_imem_ready(1'b0),
        .i_memory_wait(memory_wait),
        .i_id_ex_valid(id_ex_valid),
        .i_id_ex_predicted_taken(id_ex_predicted_taken)
    );
