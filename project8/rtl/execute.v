...
module execute (
    ...
    output wire is_pc_4, // for branch predictor update in fetch stage, to determine whether the instruction in execute stage is a control transfer instruction or not (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine if the instruction is control transfer or not)
    ...
    assign is_pc_4 = !branchjalsel && (i_Branch == 1'b1 || i_Jal == 1'b1); // for branch predictor update in fetch stage, to determine whether the instruction in execute stage is a control transfer instruction or not (for better timing, we use the signal generated in execute stage, which is used for next PC calculation in hart, to determine if the instruction is control transfer or not)
